...
def test(args, model, device, test_loader, classes):
    ...
    with torch.no_grad():  # no gradient calculations required during testing
        for imgs, labels in test_loader:
            ...
            # WandB – Log images in your test dataset automatically, along with predicted and true labels by passing
            # pytorch tensors with image data into wandb.Image example_images.append(wandb.Image( imgs[:16],
            # caption="Pred: {} Truth: {}".format([classes[pred[i].item()] for i in pred[:16]], [classes[labels[i]]
            # for i in labels[:16]])))
    # WandB – wandb.log(a_dict) logs the keys and values of the dictionary passed in and associates the values with a
    # step. You can log anything by passing it to wandb.log, including histograms, custom matplotlib objects, images,
    # video, text, tables, html, pointclouds and other 3D objects. Here we use it to log test accuracy, loss and some
...
if __name__ == '__main__':
    ...
    wandb.init(entity="yadavb", project="NIH-Eli-Lily10k")
    ...
    def main():
        ...
        # # modifying last layer to change the num of output_features to num of diseases to be classified
        # dataset # grabbing number of neurons in the second last layer
        ...
        # WandB – wandb.watch() automatically fetches all layer dimensions, gradients, model parameters and logs them
        # automatically to your dashboard. Using log="all" log histograms of parameter values in addition to gradients
        ...
        # current run.
        torch.save(model.state_dict(), "model_resnet34_cf_10000.h5")
        wandb.save('model_resnet34_cf_10000.h5')
